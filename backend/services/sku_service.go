...
func (s *SKUService) BatchCreateSKU(productID int, skus []models.ProductSKU, merchantID int) error {
	// 检查商品是否属于该商户
	var product models.Product
	...
	// 重新计算商品总库存
	var totalStock int
	if err := tx.Model(&models.ProductSKU{}).Where("product_id = ? AND status = ?", productID, "active").Select("COALESCE(SUM(stock), 0)").Scan(&totalStock).Error; err != nil {
		tx.Rollback()
		return err
	}
...
func (s *SKUService) DeleteSKU(skuID int, merchantID int) error {
	...
	s.DB.Model(&models.ProductSKU{}).Where("product_id = ? AND status = ? AND id != ?", sku.ProductID, "active", skuID).Count(&activeSKUCount)
	if activeSKUCount == 0 {
...
func (s *SKUService) GenerateSKUsFromSpecs(productID int, basePrice float64, merchantID int) ([]models.ProductSKU, error) {
	// 检查商品是否属于该商户
	var product models.Product
	result := s.DB.Where("id = ? AND merchant_id = ?", productID, merchantID).First(&product)
	if result.Error != nil {
